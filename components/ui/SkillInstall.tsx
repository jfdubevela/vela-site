...
function PlatformBox({
  ...
  return (
    <div className="rounded-[14px] border border-[rgba(247,243,235,0.12)] bg-white/[0.03] overflow-hidden">
      <button
        onClick={() => setOpen((o) => !o)}
        aria-expanded={open}
      ...
      <AnimatePresence initial={false}>
        {open && (
          ...
          >
            <div className="px-7 pb-7 border-t border-[rgba(247,243,235,0.12)]">
                ...
                {steps.map((step, i) => (
                  <li key={i} className="flex gap-4">
                    ...
                    <div className="flex flex-col gap-1">
                      <span className="text-sm font-medium text-[#E8E4DA] leading-snug">
                        {step.label}
                      </span>
                      {step.detail && (
                        <span className="text-xs text-[rgba(247,243,235,0.45)] leading-relaxed">
                          {step.detail}
                        </span>
                      )}
                    </div>
...
function ClaudeLogo() {
  return (
    <svg width="20" height="20" viewBox="0 0 24 24" fill="none" aria-label="Claude">
      <path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z" fill="rgba(247,243,235,0.75)"/>
    </svg>
  )
}
