...
export default function Tarification() {
  return (
    <section id="tarification" className="bg-[#E8E4DA] py-28 md:py-36">
      <div className="max-w-[1180px] mx-auto px-6 md:px-12 lg:px-20">
        ...
        <ScrollReveal delay={0.05}>
          <p className="font-mono text-[0.75rem] uppercase tracking-[0.16em] text-[#0A2E4D]/40 mb-6 font-mono">
                ...
                <div className="flex flex-col gap-1 mb-6">
                  ...
                  <p
                    className={`text-xs leading-relaxed ${
                      tier.highlight ? 'text-[rgba(247,243,235,0.5)]' : 'text-[#0A2E4D]/45'
                    }`}
                  >
                    {tier.desc}
                  </p>
                    ...
                    <li key={f} className="flex items-start gap-2.5">
                      ...
                      <span
                        className={`text-sm leading-relaxed ${
                          tier.highlight ? 'text-[rgba(247,243,235,0.7)]' : 'text-[#0A2E4D]/60'
                        }`}
                      >
                        {f}
                      </span>
        ...
        {/* Implantation grid */}
        <ScrollReveal delay={0.05}>
          <p className="font-mono text-[0.75rem] uppercase tracking-[0.16em] text-[#0A2E4D]/40 mb-5 font-mono">
        ...
        <div className="grid grid-cols-3 gap-3 mb-14">
          {implantationTiers.map((t, i) => (
            <ScrollReveal key={t.size} delay={i * 0.06}>
              <div className="rounded-[10px] bg-[#0A2E4D]/[0.04] border border-[#0A2E4D]/10 p-5 flex flex-col gap-2 hover:bg-[#0A2E4D]/[0.07] transition-colors">
                <div className="flex items-baseline gap-2">
                  <span className="font-mono text-lg font-medium text-[#0A2E4D]">{t.size}</span>
        ...
        <ScrollReveal delay={0.1}>
          <div className="rounded-[14px] bg-[#0A2E4D] p-8 md:p-10 mb-14">
            <div className="grid md:grid-cols-2 gap-8 items-start">
              <div>
                <h3 className="text-xl md:text-2xl font-medium text-[#E8E4DA] mb-3 leading-tight">
                  Certains projets dépassent<br />l&apos;automatisation standard.
                </h3>
                <p className="text-sm text-[rgba(247,243,235,0.5)] leading-relaxed mb-6">
              ...
              <div className="grid grid-cols-1 gap-3">
                {[
                  ...
                  <div key={item.title} className="rounded-[10px] bg-white/[0.07] border border-[rgba(247,243,235,0.12)] p-4">
                    <p className="text-sm font-semibold text-[#E8E4DA] mb-1">{item.title}</p>
                    <p className="text-xs text-[rgba(247,243,235,0.45)] leading-relaxed">{item.desc}</p>
                  </div>
                ))}
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-10">
          {maintenanceTiers.map((t, i) => (
            <ScrollReveal key={t.name} delay={i * 0.06}>
              <div className="rounded-[10px] bg-[#0A2E4D]/[0.04] border border-[#0A2E4D]/10 p-5 flex items-center justify-between hover:bg-[#0A2E4D]/[0.07] transition-colors">
                <div>
